...
public class MongoContext : IDisposable
{
    ...
    {
        var collectionAttr =
            (MongoEntityAttribute)entityType.GetTypeInfo().GetCustomAttribute(typeof(MongoEntityAttribute));

        if (collectionAttr == null)
            throw new InvalidOperationException(
                $"Entity with type '{entityType.GetTypeInfo().FullName}' is not MongoDB entity");
    ...
    private void GetClusterBulder(ClusterBuilder builder, ILogger<MongoContext> logger)
    {
        ...
        builder.Subscribe<CommandSucceededEvent>(e =>
        {
            ...
            if (e.Duration.TotalMilliseconds > 100)
                logger.LogWarning("LONG REQUEST: {CommandName}, {Duration} ms ", e.CommandName,
